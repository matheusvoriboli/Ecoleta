...
class PointsController {
    ...
    async index(request: Request, response: Response){
        ...
        .split(',')
        .map(item => Number(item.trim())) //trim permite que eu tenha espaço entre as virgulas

        const points = await knex('points')
        .join('point_items', 'points.id', '=', 'point_items.point_id' )
        .whereIn('point_items.item_id', parsedItems) // Uso o whereIn pq quero pegar os que tem pelo menos esta condição, pode ser q tenha mais
        .where('city', String(city))
        .where('uf', String(uf))
        .distinct()//Para não mostrar o mesmo ponto de coleta repetido
        .select('points.*')
